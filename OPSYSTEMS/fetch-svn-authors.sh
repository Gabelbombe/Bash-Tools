...
until [[ -z "$1" ]]; do
  ...
  case $parameter in
    ...
    e )            email_domain=$value;;
    email-domain ) email_domain=$value;;
...
# If an emails domain exists run secondary parser.
if [ -n "$domain" ]; then
  # Assumnes /usr/bin/awk
  /bin/cat $tmp_file | awk -v r=$root '{print $1" <" $1 r ">"}'
fi
